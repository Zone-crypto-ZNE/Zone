...
// Main hash function for Zone (replacing Keccak)
void hash_process(hash_state *state, const uint8_t *buf, size_t count) {
    sha256_ctx ctx;
    sha256_init(&ctx);
    sha256_update(&ctx, buf, count);
    sha256_final(&ctx, state->h);
}

// Hash a fixed-size block (32 bytes)
void hash_permutation(hash_state *state) {
    sha256_ctx ctx;
    sha256_init(&ctx);
    sha256_update(&ctx, state->h, HASH_SIZE);
    sha256_final(&ctx, state->h);
}

// Tree hash functions (simplified for Arduino)
void hash_tree(const uint8_t *data, size_t count, uint8_t *hash) {
    if (count == 1) {
    ...
    for (i = 0; i < count - 1; i += 2) {
        memcpy(buffer, &data[i * HASH_SIZE], HASH_SIZE);
        memcpy(buffer + HASH_SIZE, &data[(i + 1) * HASH_SIZE], HASH_SIZE);
        sha256_ctx ctx;
        sha256_init(&ctx);
        sha256_update(&ctx, buffer, 2 * HASH_SIZE);
        sha256_final(&ctx, &data[i / 2 * HASH_SIZE]);
    }
    if (count % 2) {
        memcpy(&data[(count / 2) * HASH_SIZE], &data[(count - 1) * HASH_SIZE], HASH_SIZE);
    }
    hash_tree(data, (count + 1) / 2, hash);
}

// Extra hash functions (simplified to SHA-256 variants)
